...
func (h *ArticleHandler) GetAllArticles(c *gin.Context) {
	...
	_, articles, pages, total, err := h.articleService.GetAllPressAndArticles(page_int, size_int, with_pagination_bool, "article", sorting)
	if err != nil {
	...
	if with_pagination_bool {
		c.JSON(http.StatusOK, gin.H{
			"data": dto.ToArticleResponseDTOs(articles, base_url),
			"pagination": gin.H{
...
// @Summary Delete main photo from article
// @Description Удаляет главную/preview фотографию к статье
// @Accept json
// @Produce json
// @Tags Articles
// @Param id path int true "Article ID"
// @Param is_preview query bool false "Is Preview"
// @Success 200 {object} map[string]string
// @Router /api/articles/{id}/main_photo [delete]
func (h *ArticleHandler) DeleteMainPhotoFromArticle(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid id format"})
		return
	...
	err = h.articleService.DeleteMainOrPreviewPhoto(uint(id), "article", is_preview)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
